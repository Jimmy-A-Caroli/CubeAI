...
class PickReviewAnnotation:
    ...
    def __post_init__(self) -> None:
        ...
        if self.strategy_ref is not None and (
            not isinstance(self.strategy_ref, str) or not self.strategy_ref.strip()
        ):
        ...
        if any(not isinstance(reason, ReviewReason) for reason in reasons) or len(
            set(reasons)
        ) != len(reasons):
        ...
        if self.suggested_rating is not None and (
            not isinstance(self.suggested_rating, (int, float))
            or isinstance(self.suggested_rating, bool)
            or not isfinite(self.suggested_rating)
        ):
