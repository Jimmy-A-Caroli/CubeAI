...
def allocate_packs(
    draft_id: str, version: CubeVersion, configuration: DraftConfiguration
) -> tuple[AllocatedPack, ...]:
    ...
    for pack_number in range(configuration.seats * configuration.packs_per_seat):
        ...
        cards = tuple(
            DraftCardInstance(f"{draft_id}:card:{pack_number}:{i}", draft_id, card.id)
            for i, card in enumerate(shuffled[start : start + configuration.pack_size])
        )
        result.append(
            AllocatedPack(
