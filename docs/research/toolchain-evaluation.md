...
|---|---|---:|---|---|
| Python | Runtime | 3.14.0 | `>=3.14,<3.15`; pin major/minor `3.14` in developer and CI setup | The installed patch is selected by the platform/setup tool, not `uv.lock`. Python 3.14 is the current bugfix series; 3.15 is pre-release. [Python downloads](https://www.python.org/downloads/), [support status](https://devguide.python.org/versions/) |
| uv | Python project/lock tool | 0.12.7 | Exact tool pin `0.12.7` in bootstrap and CI | `uv.lock` resolves application/dev dependencies exactly. uv documents possible breaking changes, including lock schema changes, at minor releases. [installation](https://github.com/astral-sh/uv/blob/main/docs/getting-started/installation.md), [versioning](https://docs.astral.sh/uv/reference/policies/versioning/) |
| FastAPI | HTTP boundary | 0.141.1 | `>=0.141.1,<1` | Exact package and transitive versions in `uv.lock`; 0.141.1 is the current release in official release notes. [releases](https://fastapi.tiangolo.com/release-notes/) |
| Pydantic | Boundary validation | 2.13.4 | `>=2.13.4,<3` | Exact resolution in `uv.lock`; the 2.13 line supports Python 3.14. [changelog](https://docs.pydantic.dev/latest/changelog/) |
| pytest | Backend tests | 9.1.1 | `>=9.1.1,<10` | Exact resolution in `uv.lock`. [release announcements](https://docs.pytest.org/en/stable/announce/index.html), [compatibility policy](https://docs.pytest.org/en/stable/backwards-compatibility.html) |
| Ruff | Backend lint/format | 0.16.0 | `>=0.16.0,<0.17` | Exact resolution in `uv.lock`; the narrow range reflects Ruff's documented pre-1.0 minor-version policy. [releases](https://github.com/astral-sh/ruff/releases), [versioning](https://docs.astral.sh/ruff/versioning/) |
| mypy | Backend type checker | 2.3.1 | `>=2.3.1,<3` | Exact resolution in `uv.lock`; run in strict mode. [changelog](https://mypy.readthedocs.io/en/stable/changelog.html) |
| Import Linter | Import boundaries | 2.14 | `>=2.14,<3` | Exact resolution in `uv.lock`; 2.14 is the current release and the project documents Python 3.14 support. [release notes](https://import-linter.readthedocs.io/en/stable/release_notes/) |
...
|---|---|---|
| Backend install | `uv sync --locked --all-groups` | Reproduce the committed environment and fail on a stale lockfile. |
| Backend format | `uv run ruff format --check backend` | Check formatting without edits. |
| Backend lint | `uv run ruff check backend` | Static lint checks. |
| Backend types | `uv run mypy --strict backend/src backend/tests` | Strict static type check. Prefer storing equivalent strict settings in `pyproject.toml` as well. |
| Backend boundaries | `uv run lint-imports` | Enforce domain/application/outer-layer import contracts. |
| Backend tests | `uv run pytest -q backend/tests` | Deterministic backend unit and contract tests. |
| Backend package | `uv build` | Prove that the `src` package builds without relying on checkout import leakage. |
...
| Whole repository | `uv run python scripts/validate.py` | Run the same ordered checks locally and in CI, returning the first diagnosable failure. |

The root validator should use Python's subprocess API with explicit argument lists rather than shell-specific syntax, so the same entry point works on macOS, Linux, and Windows. It should print each child command and elapsed time, preserve exit codes, avoid network access after locked installation, and never mutate formatting. M0-006 should be the first task to create that script.

## Lockfiles

Commit `pyproject.toml` plus `uv.lock` for the backend and `package.json` plus `package-lock.json` for the frontend. uv describes `uv.lock` as a universal cross-platform lockfile with exact resolved versions and says it should be committed; `uv sync --locked` validates freshness and refuses to change it. `--frozen` is not the CI default because it can use the lock without checking whether project metadata changed. [uv lockfile](https://docs.astral.sh/uv/concepts/projects/layout/#the-lockfile), [uv sync](https://docs.astral.sh/uv/concepts/projects/sync/)

Pin uv itself to `0.12.7` in documented bootstrap and CI configuration because uv's minor versions can change behavior and the lock schema. Keep dependency declaration ranges in `pyproject.toml` for compatibility intent while allowing only explicit, reviewed `uv lock --upgrade...` changes to rewrite resolutions. The standards-based escape hatch is `uv export --format pylock.toml`; the fallback pip workflow would use a `venv` and the experimental `pip lock`, with separate lock output where platforms differ. [uv export](https://docs.astral.sh/uv/concepts/projects/layout/#relationship-to-pylocktoml), [pip lock](https://pip.pypa.io/en/stable/cli/pip_lock/), [pyproject specification](https://packaging.python.org/en/latest/specifications/pyproject-toml/)

Commit npm's root frontend `package-lock.json`, declare `packageManager: "npm@11.19.1"`, and use only `npm ci` in CI. npm documents that `npm ci` requires an existing lock, removes a pre-existing `node_modules`, fails when the manifest and lock disagree, and never writes the lock. Commit any project-affecting npm flags in `frontend/.npmrc` so CI uses the same tree-building settings. [npm ci](https://docs.npmjs.com/cli/v11/commands/npm-ci/), [package-lock format](https://docs.npmjs.com/cli/v11/configuring-npm/package-lock-json/)
...
- Pin Python `3.14`, Node `24`, npm `11.19.1`, and uv `0.12.7`; locked installs must complete before `uv run python scripts/validate.py`.
- Cache download/package-manager data, not `.venv` or `node_modules`. Key uv cache entries by runner OS, architecture, Python line, uv version, and `hashFiles('uv.lock')`; key npm cache entries by runner OS, architecture, Node/npm lines, and `hashFiles('frontend/package-lock.json')`. Official guidance demonstrates lockfile hashes in cache keys and warns that cache content can be executable. [GitHub cache reference](https://docs.github.com/en/actions/reference/workflows-and-actions/dependency-caching), [cache security](https://docs.github.com/en/actions/concepts/workflows-and-actions/dependency-caching), [uv caching](https://docs.astral.sh/uv/guides/integration/github/#caching)
- Run the root validation command unchanged across local and CI environments. Platform smoke jobs may run locked installs plus the root command; they must not have privileged Docker or deployment access.
...
1. Python 3.14 with uv 0.12.7, standard `pyproject.toml`, committed `uv.lock`, FastAPI/Pydantic only at the HTTP boundary, pytest, Ruff, mypy strict mode, and Import Linter.
2. Node 24 with npm 11.19.1, committed `package-lock.json`, React 19.2, strict TypeScript 6.0, Vite 8.2, Vitest 4.1, React Testing Library, ESLint/typescript-eslint for correctness, and exact Prettier 3.9.6 for formatting.
