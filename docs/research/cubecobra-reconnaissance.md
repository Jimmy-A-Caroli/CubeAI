...
| CardDetails exposes separate nested `scryfall_id` and `oracle_id` fields. | Yes, in CardDetails type | Both present on one normal card | Completeness and custom-card behavior unknown | [Card datatype](https://github.com/dekkerglen/CubeCobra/blob/master/packages/utils/src/datatypes/Card.ts) | Pass them as distinct provider evidence; validate their printing/Oracle semantics during Scryfall resolution, and diagnose missing or inconsistent identity. |
