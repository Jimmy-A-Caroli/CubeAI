...
- **Accepted policy:** `raw-ranking-v0` uses Oracle-ID lookup, a documented `2.6` mid-tier fallback, ascending `DraftCardInstance.id` ties, and strategy/version provenance. The approved `cubeai-raw-ranking-v0@2026.09.04.1` artifact is a CubeAI-owned, versioned transformation with 480 exact Oracle-ID mappings for the 540-card `modovintage` target snapshot and 60 explicit fallback cases. CubeCobra/CubeCobraML data are prohibited until explicit reusable-data rights are confirmed; 17Lands is not a Bot v0 Vintage Cube substitute.
